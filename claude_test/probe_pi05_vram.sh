#!/usr/bin/env bash
# probe_pi05_vram.sh -- Run a short Pi0.5 fine-tune at the given
# per-GPU batch_size and report peak VRAM. Mirrors probe_pi0_vram.sh
# with pi05-specific policy options. Probe only; production isolated.
#
# Usage: probe_pi05_vram.sh <per_gpu_batch_size>
#   STEPS=<n>     override step count (default 50)
#
# Outputs:
#   claude_test/probe_logs/vram_pi05_b${BATCH}.csv  nvidia-smi 1 Hz
#   claude_test/probe_logs/train_pi05_b${BATCH}.log accelerate stdout
#   outputs/train/fr5_pi05_vram_probe_b${BATCH}/    cleaned each run
#
# See ToDo 2026-05-01 / GitHub issue #58.
...
# Mirror conda discovery from 7__train_pi05_adv.sh.
_conda_sh=""
...
JOB_NAME="fr5_pi05_vram_probe_b${BATCH}"
OUTPUT_DIR="outputs/train/${JOB_NAME}"
LOG_DIR="claude_test/probe_logs"
mkdir -p "${LOG_DIR}"
VRAM_LOG="${LOG_DIR}/vram_pi05_b${BATCH}.csv"
TRAIN_LOG="${LOG_DIR}/train_pi05_b${BATCH}.log"
